...
if (!JWT_SECRET) { throw new Error('FATAL: JWT_SECRET environment variable is not set. Server cannot start securely.'); }
...
export class AppGateway {
  ...
  // ============================================
  // CHAT ENGAGEMENT EVENTS (Rain, Tips, Trivia)
  // ============================================

  /**
   * Broadcast rain event to chat room
   */
  public broadcastRain(data: { rainId: string; totalAmount: number; amountPerUser: string; participants: Array<{ userId: string; username: string; amount: string }> }): void {
    this.io.of("/casino").to("room:chat").emit("chat:rain", data);
    // Send balance updates to each participant
    for (const p of data.participants) {
      this.io.of("/casino").to(`user:${p.userId}`).emit("balance:update", {
        userId: p.userId,
        currency: "USDT",
        change: parseFloat(p.amount),
        transactionType: "RAIN_RECEIVED",
      });
    }
  }

  /**
   * Broadcast tip event to chat room
   */
  public broadcastTip(data: { from: { userId: string; username: string }; to: { userId: string; username: string }; amount: string }): void {
    this.io.of("/casino").to("room:chat").emit("chat:tip", data);
    // Balance updates
    this.io.of("/casino").to(`user:${data.from.userId}`).emit("balance:update", {
      userId: data.from.userId,
      currency: "USDT",
      change: -parseFloat(data.amount),
      transactionType: "TIP_SENT",
    });
    this.io.of("/casino").to(`user:${data.to.userId}`).emit("balance:update", {
  ...
  /**
   * Broadcast trivia question to chat room
   */
  public broadcastTrivia(data: { id: string; question: string; prize: number }): void {
    this.io.of("/casino").to("room:chat").emit("chat:trivia", data);
  }

  /**
   * Broadcast trivia winner to chat room
   */
  public broadcastTriviaWinner(data: { id: string; winnerId: string; winnerUsername: string; prize: number; question: string }): void {
    this.io.of("/casino").to("room:chat").emit("chat:trivia-winner", data);
    // Balance update for winner
    this.io.of("/casino").to(`user:${data.winnerId}`).emit("balance:update", {
      userId: data.winnerId,
      currency: "USDT",
      change: data.prize,
      transactionType: "TRIVIA",
    });
  }
