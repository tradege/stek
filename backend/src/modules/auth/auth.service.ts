...
export class AuthService {
  ...
  async register(dto: RegisterDto, siteId?: string): Promise<AuthResponse> {
    ...
    const user = await this.prisma.$transaction(async (tx) => {
      const newUser = await tx.user.create({
        data: {
          ...
          passwordHash,
          role: UserRole.USER,
          status: UserStatus.PENDING_VERIFICATION, // CHANGED: Now PENDING_VERIFICATION
          parentId,
          hierarchyPath,
          hierarchyLevel,
          displayName: username,
          siteId: effectiveSiteId,
          emailVerificationToken: verificationCode, // Store the 6-digit code
        },
      });
      ...
      // Create verification token record with expiry (15 minutes)
    ...
    this.logger.log(`[EMAIL VERIFICATION] User ${user.email} - Code: ${verificationCode}`);

    // Send verification email with timeout protection
    const emailPromise = this.emailService.sendVerificationEmail(user.email, verificationCode, effectiveSiteId)
      .then(() => {
        this.logger.log(`[EMAIL VERIFICATION] Verification email sent to ${user.email}`);
        return true;
      })
      .catch((emailError) => {
        this.logger.error(`[EMAIL VERIFICATION] Failed to send email to ${user.email}: ${emailError.message}`);
        return false;
      });

    // Wait for email with 10-second timeout
    const timeoutPromise = new Promise<boolean>((resolve) => {
      setTimeout(() => {
        this.logger.warn(`[EMAIL VERIFICATION] Email sending timed out for ${user.email}`);
        resolve(false);
      }, 10000);
    });
    ...
    if (!emailSent) {
      this.logger.warn(`[EMAIL VERIFICATION] Email may not have been sent to ${user.email}, but registration continues`);
    }
  ...
  async verifyEmail(email: string, code: string): Promise<{ success: boolean; message: string }> {
    ...
    this.logger.log(`[EMAIL VERIFIED] User ${user.email} is now ACTIVE`);

    // Send welcome email (non-blocking)
    this.emailService.sendWelcomeEmail(user.email, user.username, user.siteId)
  ...
  async resendVerificationCode(email: string): Promise<{ success: boolean; message: string }> {
    ...
    if (!user) {
      ...
      return { success: true, message: 'If the email exists, a new verification code has been sent.' };
    }
    ...
    this.logger.log(`[EMAIL VERIFICATION RESEND] User ${user.email} - Code: ${verificationCode}`);

    // Send email with timeout
    const emailPromise = this.emailService.sendVerificationEmail(user.email, verificationCode, user.siteId)
      .then(() => true)
      .catch((err) => {
  ...
  // ============================================
  // LOGIN (BLOCKS PENDING_VERIFICATION USERS)
  // ============================================
  ...
  async login(dto: LoginDto, siteId?: string, ip?: string, userAgent?: string): Promise<AuthResponse> {
    ...
    // HANDLE PENDING_VERIFICATION USERS - send new code and return structured response
    if (user.status === UserStatus.PENDING_VERIFICATION) {
      // Verify password first before sending a new code
      let isPwValid = false;
      if (user.passwordHash.startsWith('$argon2')) {
        isPwValid = await argon2.verify(user.passwordHash, password);
      } else {
        isPwValid = await bcrypt.compare(password, user.passwordHash);
      }
      if (!isPwValid) {
        throw new UnauthorizedException('Invalid credentials');
      }
      ...
      try {
        await this.resendVerificationCode(user.email);
        this.logger.log(`[LOGIN] Auto-sent new verification code to unverified user ${user.email}`);
      } catch (e) {
        this.logger.error(`[LOGIN] Failed to auto-send verification code: ${e.message}`);
      }

      // Return a 200 response with pendingVerification flag (NOT an error)
      return {
        user: this.sanitizeUser(user),
        token: '',
  ...
  async enable2FA(userId: string): Promise<{ secret: string; qrCode: string }> {
    ...
    const otpauthUrl = authenticator.keyuri(user.email, 'Betworkss', secret);
  ...
  async getMe(userId: string): Promise<UserWithBalance> {
    ...
    return {
      ...
      balance: filteredWallets.map((wallet) => ({
        ...
        available: (Number(wallet.balance) + Number(wallet.bonusBalance || 0)).toString(),
