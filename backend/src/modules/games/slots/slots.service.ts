...
const MIN_BET = 0.10;
const MAX_BET = 1000;
// HOUSE_EDGE is now fetched dynamically from DB via getGameConfig
...
export class SlotsService {
  ...
  async spin(userId: string, dto: SlotSpinDto, siteId: string = '1') {
    ...
    const gameConfig = await getGameConfig(this.prisma, siteId, 'slots');
    const dynamicHouseEdge = gameConfig.houseEdge;

    const spinResult = mode.spin(serverSeed, clientSeed, nonce, betAmount, { ...options, houseEdge: dynamicHouseEdge });

    // Cap total win
    const maxWin = Math.min(betAmount * 5000, gameConfig.maxPayoutPerBet);
    const totalWin = Math.min(spinResult.totalPayout, maxWin);
    ...
    this.postBetProcessing(userId, betRecord.id, betAmount, totalWin, dynamicHouseEdge, siteId, gameMode);
