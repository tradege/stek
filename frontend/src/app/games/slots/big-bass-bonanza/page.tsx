...
const SYMBOL_CONFIG: Record<string, { emoji: string; label: string; color: string; tier: string }> = {
  fish_mega:   { emoji: "🐋", label: "Mega Fish",  color: "#FFD700", tier: "premium" },
  fish_large:  { emoji: "🐟", label: "Large Fish", color: "#00BCD4", tier: "premium" },
  fish_medium: { emoji: "🐠", label: "Med Fish",   color: "#FF6D00", tier: "mid" },
  fish_small:  { emoji: "🐡", label: "Small Fish", color: "#FFB300", tier: "mid" },
  ace:         { emoji: "🅰️", label: "Ace",        color: "#FF1744", tier: "low" },
  ...
  ten:         { emoji: "🔟", label: "Ten",        color: "#78909C", tier: "low" },
  fisherman:   { emoji: "🎣", label: "Fisherman",  color: "#FFD700", tier: "special" },
  scatter:     { emoji: "🎣", label: "Fisherman",  color: "#FFD700", tier: "special" },
...
export default function BigBassBonanzaPage() {
  ...
  const [fishCollection, setFishCollection] = useState<{ emoji: string; value: number }[]>([]);
  const [totalFishValue, setTotalFishValue] = useState(0);
  const [instantHook, setInstantHook] = useState(false);
  ...
  useEffect(() => {
    ...
    if (!token) return;
    fetch(`${API_URL}/api/slots/history?gameMode=BIG_BASS&limit=10`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/slots/spin`, {
        ...
        body: JSON.stringify({ gameMode: "BIG_BASS", betAmount: freeSpinSession ? 0 : bet }),
      });
      ...
      setTimeout(() => {
        ...
        // Fish collection
        const fishFeature = data.features?.find((f: any) => f.type === "fish_catch" || f.type === "instant_hook");
        if (fishFeature) {
          if (fishFeature.type === "instant_hook") {
            setInstantHook(true);
            setFishCollection(prev => [...prev, { emoji: "🐟", value: fishFeature.value || 0 }]);
            setTotalFishValue(prev => prev + (fishFeature.value || 0));
          } else if (fishFeature.fish) {
            const newFish = fishFeature.fish.map((f: any) => ({ emoji: "🐟", value: f.value || 0 }));
            setFishCollection(prev => [...prev, ...newFish]);
            setTotalFishValue(prev => prev + newFish.reduce((s: number, f: any) => s + f.value, 0));
          }
        }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0A0E17] flex items-center justify-center">
        <div className="text-center"><div className="text-6xl mb-4 animate-bounce">🐟</div><div className="text-white font-bold">Loading Big Bass Bonanza...</div></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0A0E17] text-white">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-15">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 100%, rgba(0,188,212,0.15) 0%, transparent 60%), radial-gradient(ellipse at 20% 30%, rgba(0,119,182,0.1) 0%, transparent 50%)" }} />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                🐟 Big Bass Bonanza
                <span className="text-xs bg-teal-500/20 text-teal-400 px-2 py-0.5 rounded-full font-bold">COLLECT</span>
              </h1>
              <p className="text-sm text-gray-400">5x3 • Fish Collection • Instant Hook • 96% RTP</p>
            </div>
          </div>
        ...
        {showPaytable && (
          <div className="bg-[#131B2C] rounded-xl border border-teal-500/20 p-6 mb-6">
            <h3 className="text-lg font-bold text-teal-400 mb-4">Paytable - Fish Collection</h3>
            <p className="text-sm text-gray-400 mb-4">Catch fish to collect values! Fisherman scatter triggers free spins where all fish are collected.</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {Object.entries(SYMBOL_CONFIG).filter(([k]) => k !== "scatter" && k !== "wild").map(([key, cfg]) => (
                <div key={key} className="bg-[#0A0E17] rounded-lg p-3 border border-teal-900/20">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="bg-[#0A0E17] rounded-lg p-3 border border-yellow-500/30">
                <span className="text-2xl">🎣</span>
                <span className="text-sm text-yellow-400 font-bold ml-2">Fisherman</span>
                <div className="text-xs text-gray-400 mt-1">3+ triggers Free Spins</div>
              </div>
              <div className="bg-[#0A0E17] rounded-lg p-3 border border-teal-500/30">
                <span className="text-2xl">🪝</span>
                <span className="text-sm text-teal-400 font-bold ml-2">Instant Hook</span>
                <div className="text-xs text-gray-400 mt-1">Random catch in base game!</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          <div className="space-y-4">
            <SlotFrame spinning={spinning} isWin={totalWinDisplay > 0} isBigWin={showBigWin} gameName="Big Bass Bonanza" gameTheme="ocean">
              <BigWinPopup show={showBigWin} amount={bigWinAmount} multiplier={bigWinMultiplier} onClose={() => setShowBigWin(false)} gameTheme="ocean" />

              {/* Instant Hook Alert */}
              {instantHook && (
                <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 bg-gradient-to-r from-teal-600 to-cyan-500 rounded-full px-6 py-2 text-white font-bold text-sm animate-bounce shadow-lg shadow-teal-500/40">
                  🪝 INSTANT HOOK!
                </div>
              )}
              ...
              <div className="grid grid-cols-5 gap-1.5 sm:gap-2">
                {displayGrid.map((cell, index) => {
                  ...
                  return (
                    <ErrorBoundary key={index} gameName="BigBass">
                      <ReelCell
                        symbol={cell.symbol}
              ...
              {totalWinDisplay > 0 && !showBigWin && (
                <div className="mt-3 text-center py-2 bg-gradient-to-r from-teal-500/20 via-cyan-500/30 to-teal-500/20 rounded-lg border border-teal-500/30">
                  <span className="text-teal-400 font-bold text-lg">WIN: ${totalWinDisplay.toFixed(2)}</span>
                </div>
              )}
            </SlotFrame>
          ...
          <div className="space-y-4">
            {/* Free Spins + Fish Collection */}
            {freeSpinSession && (
              <div className="bg-[#131B2C] rounded-xl border border-teal-500/40 p-4">
                <div className="text-center">
                  <div className="text-2xl mb-1">🎣</div>
                  <div className="text-lg font-bold text-teal-400">FISHING SPREE!</div>
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <div><div className="text-gray-400 text-xs">Remaining</div><div className="text-teal-400 font-bold text-lg">{freeSpinsRemaining}</div></div>
            ...
            {/* Fish Tank */}
            <div className="bg-[#131B2C] rounded-xl border border-teal-500/20 p-4">
              <h3 className="text-sm font-bold text-teal-400 mb-3 flex items-center gap-2">🐟 Fish Tank</h3>
              {fishCollection.length > 0 ? (
                <div>
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {fishCollection.slice(-12).map((fish, i) => (
                      <div key={i} className="bg-teal-500/10 border border-teal-500/20 rounded-lg px-2 py-1 text-xs flex items-center gap-1">
                        <span>{fish.emoji}</span>
                        <span className="text-teal-300 font-bold">${fish.value.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t border-white/5">
                    <span className="text-xs text-gray-400">{fishCollection.length} fish caught</span>
                    <span className="text-teal-400 font-bold">${totalFishValue.toFixed(2)}</span>
                  </div>
                </div>
              ) : (
                <div className="text-center py-4">
                  <div className="text-3xl mb-2 opacity-30">🐟</div>
                  <p className="text-xs text-gray-500">No fish caught yet. Spin to start fishing!</p>
                </div>
              )}
            </div>

            <ControlBar betAmount={betAmount} setBetAmount={setBetAmount} onSpin={handleSpin} spinning={spinning} disabled={!user}
              freeSpinSession={freeSpinSession} freeSpinsRemaining={freeSpinsRemaining} gameTheme="ocean" spinLabel={`🐟 SPIN - $${parseFloat(betAmount).toFixed(2)}`} />
            ...
            <div className="bg-[#131B2C] rounded-xl border border-white/10 p-4">
              ...
              <div className="space-y-2 text-xs text-gray-400">
                ...
                <div className="flex justify-between"><span>Mechanic</span><span className="text-teal-400">Fish Collection</span></div>
                <div className="flex justify-between"><span>RTP</span><span className="text-white">96.00%</span></div>
                <div className="flex justify-between"><span>Special</span><span className="text-teal-400">Instant Hook</span></div>
                <div className="flex justify-between"><span>Free Spins</span><span className="text-white">3+ Fisherman</span></div>
