...
const NovaRushGame = dynamic(
  () => import('@/components/games/NovaRushGame'),
  {
    ssr: false,
    loading: () => (
      <div className="bg-[#050020] rounded-xl border border-purple-800/30 p-8 flex flex-col items-center justify-center min-h-[400px]">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-400 mb-4"></div>
        <p className="text-gray-400 text-lg">Initializing Nova Rush...</p>
        <p className="text-gray-500 text-sm mt-2">Preparing Spaceship & Weapons</p>
      </div>
    ),
  }
);
...
export default function NovaRushPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Nova Rush</h1>
        <p className="text-text-secondary">Navigate through the asteroid field — cash out before a meteor hits!</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-bg-card border border-white/10 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-blue-400 tabular-nums">{formatNumber(gameStats.totalWagered)}</p>
        ...
        <div className="bg-bg-card border border-white/10 rounded-xl p-4 text-center">
          ...
          <p className="text-sm text-text-secondary">Flights Completed</p>
        </div>
        <div className="bg-bg-card border border-white/10 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-yellow-400 tabular-nums">{gameStats.highestWin.toFixed(2)}x</p>
          <p className="text-sm text-text-secondary">Highest Escape</p>
        </div>
        <div className="bg-bg-card border border-white/10 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-purple-400 tabular-nums">{gameStats.activePlayers.toLocaleString()}</p>
          <p className="text-sm text-text-secondary">Active Pilots</p>
        </div>
      </div>
    </div>
  );
